...
internal enum class ReaderSingleTapAction {
    PREVIOUS_PAGE,
    NEXT_PAGE,
    PASS_THROUGH
}
...
internal object ReaderPageLogic {
    ...
    fun resolveSingleTapAction(x: Float, width: Int): ReaderSingleTapAction {
        if (width <= 0) return ReaderSingleTapAction.PASS_THROUGH

        val leftZone = width * 0.33f
        val rightZone = width * 0.67f
        return when {
            x <= leftZone -> ReaderSingleTapAction.PREVIOUS_PAGE
            x >= rightZone -> ReaderSingleTapAction.NEXT_PAGE
            else -> ReaderSingleTapAction.PASS_THROUGH
        }
    }
