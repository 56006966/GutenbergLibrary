...
class BookLoadingDecisionsTest {
    ...
    fun `local cover is still allowed when offline`() {
        assertTrue(
            shouldLoadRemoteCover(
                localCover = "/data/user/0/com.example.projectgutenberg/files/book_1_cover.jpg",
                primaryCover = "/data/user/0/com.example.projectgutenberg/files/book_1_cover.jpg",
                networkAvailable = false
            )
        )
    }
